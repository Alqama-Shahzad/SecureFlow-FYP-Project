...
// Grouped Permission Definitions matching our systems structure
...
export default function CreateRole() {
  ...
  const [selectedPermissions, setSelectedPermissions] = useState<string[]>([
    "dashboard.read", "projects.read", "tasks.read"
  ]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!name.trim() || !description.trim()) {
      setError("Please outline a Role Title and detailed description.");
      return;
    }
    ...
    setError(null);
    try {
      await SecureFlowApiService.createRole({
        name,
        description,
        color,
        accessLevel,
      ...
      setError(err?.message || "Failed to catalog custom role specifications.");
      setIsLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      <div>
        <h1 className="text-xl font-bold text-white uppercase tracking-tight flex items-center gap-2">
          Compile Custom RBAC Role
        </h1>
        <p className="text-xs text-slate-400 mt-1">
          Formulate a specialized system clearance level. Assign selective gates and preview coverage immediately.
        </p>
      </div>
      ...
      {error && (
        <div className="bg-rose-500/15 border border-rose-500/20 rounded-xl p-4 text-xs font-mono text-rose-400 flex items-center gap-2.5">
          ...
          <span>
            <strong>COMPILE FAULT:</strong> {error}
          </span>
        </div>
      )}
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="md:col-span-8 space-y-4">
            ...
            <div>
              ...
              <input
                ...
                className="w-full bg-[#03060c] border border-slate-800 text-slate-150 placeholder-slate-650 rounded-lg px-3.5 py-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all font-sans font-bold"
              />
            </div>
            ...
            <div>
              <label className="block text-[10px] font-semibold text-slate-500 uppercase tracking-wider mb-1.5 font-mono">
                Privilege Purpose & Boundaries Description <span className="text-rose-500">*</span>
              </label>
              <textarea
                required
                value={description}
                onChange={(e) => setDescription(e.target.value)}
                placeholder="State the operations boundaries and purpose of this custom clearance level inside the systems architecture..."
                rows={4}
                className="w-full bg-[#03060c] border border-slate-800 text-slate-200 placeholder-slate-650 rounded-lg px-3.5 py-2.5 text-xs focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all font-sans leading-relaxed"
              />
            </div>
          </div>
        ...
        <div className="bg-[#090e1a]/80 backdrop-blur-md border border-slate-900 rounded-2xl p-5 sm:p-6 shadow-xl space-y-5">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-900 pb-3 gap-3">
            <div className="flex items-center gap-2 text-indigo-400">
              ...
              <h3 className="text-[10px] font-extrabold uppercase font-mono tracking-widest">
                2. Allocated System Access Rights
              </h3>
            </div>
        ...
        <div className="flex items-center justify-end gap-3 bg-[#090e1a]/85 border border-slate-900 rounded-2xl p-4 shadow-md backdrop-blur-md">
          ...
          >
            <Save size={13} />
            {isLoading ? "SAVING MATRIX LEVEL..." : "CATALOG PRIVILEGE PROFILE"}
          </button>
        </div>
